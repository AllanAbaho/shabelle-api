...
class OrderController extends Controller
{
    public function store(Request $request, $set_paid = true)
    {
        ...
        foreach ($seller_products as $seller_product) {
            ...
            $order->code = date('Ymd-His') . rand(10, 99) . $user->user_name;
    ...
    public function paySeller()
    {
        ...
        foreach ($orders as $order) {
            ...
            $seller = User::where('id', $order->seller_id)->where('user_type', 'seller')->first();
            $customer = User::where('id', $order->user_id)->where('user_type', 'customer')->first();
            if(!$seller){
                Log::info('Seller not found for order '.$order->id);
                continue;
            }
            if(!$customer){
                Log::info('Customer not found for order '.$order->id);
                continue;
            }
            $toAccount = $seller->account_number;
            $amount = strval($order->grand_total);
            $fromAccount = $customer->account_number;
            $phoneNumber = $customer->phone;
            $senderName = $customer->name;
            $sellerName = $seller->name;
            $milliseconds = floor(microtime(true) * 1000);
            $transactionId = $milliseconds . $customer->user_name;
            try {
                $url = BRIDGE_API . PAY_SELLER;
                $post_data = [
                    "toAccount" => $toAccount,
                ...
                Log::info('Debit Error Exception Error', [$e->getMessage()]);
                return (['success' => false, 'message' => $e->getMessage()]);
            }
        }
        Log::info($count. ' Orders paid for successfully');
        return (['success' => true, 'message' => $count. ' Orders paid for successfully']);
    }
}
